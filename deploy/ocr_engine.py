...
ОПРЕДЕЛЕНИЕ ТИПА ПО РУКОПИСНОЙ ПОМЕТКЕ НА ЧЕК:
  "узкий" / "уз" / "у/з" / "без мидр" / "не широкий" / "не шир" / "narrow" → n_sph, n_cyl, n_ax
  "широкий" / "шир" / "ш/з" / "цикло" / "с мидр" / "мидр" / "не узкий" / "wide" / "cyclo" → c_sph, c_cyl, c_ax
...
══ PENTACAM / ТОМОГРАФИЯ ══
  Ant. Axial Cyl/Ax → p_ant_c, p_ant_a (ВСЕГДА в плюс-нотации)
  Post. Axial Cyl/Ax → p_post_c, p_post_a (ВСЕГДА в плюс-нотации)
  Total/Net Cyl/Ax → p_tot_c, p_tot_a (ВСЕГДА в минус-нотации, ось слабого меридиана)

══ ПРАВИЛА ══
1. Запятую → точка: "27,63" → 27.63. Плюс писать как число: "+1,75" → 1.75
1а. ТИЛЬДА "~": декоративный разделитель на чеках авторефрактометра. ИГНОРИРУЙ её, бери только число после неё.
    "~-2.75  ~-0.75  ~165" → sph:-2.75, cyl:-0.75, axis:165. "S~+0.25" → 0.25
...
1а. ТИЛЬДА "~": декоративный символ, ИГНОРИРУЙ её, бери только число после неё. "~24.03" → 24.03
...
- ТИЛЬДА "~": встречается на чеках авторефрактометра перед ЛЮБЫМ числом как декоративный разделитель.
  ВСЕГДА игнорируй тильду и бери только цифру после неё.
  Примеры: "~-2.75" → -2.75,  "~-0.75" → -0.75,  "~165" → 165,  "S~+0.25" → 0.25
  Строка "~-2.75  ~-0.75  ~165" → sph:-2.75, cyl:-0.75, axis:165
...
### ПОЛЕ: autoref_narrow
Строка/секция: "Автореф (узкий зрачок)" / "Авторефкератометрия на узкий зрачок" / "Авторефрактометр узк" / "Ref узк" / "S C A" таблица чека без пометки или с пометкой "узкий" / "narrow" / "у/з".
...
### ПОЛЕ: biometry
  "APL" / "Axial Length" / "ПЗО" / "AL" → al (мм)
  "ACD" / "ГПК" / "Глубина передней камеры" → acd (мм)
  "LT" / "Хрусталик" / "Толщина хрусталика" → lt (мм)
  "CCT" / "Толщина роговицы" / "Пахиметрия" → cct (мкм)
...
### ПОЛЕ: autoref_wide
Строка/секция: "Автореф (циклоплегия)" / "Авторефкератометрия на широкий зрачок" / "Авторефр широкий" / "Mydriasis" / "Цикло" / "Ref цикло" / "wide" / "ш/з".
На чеке: итоговая строка чека с пометкой "широкий"/"цикло"/"шир"/"мидр"/"wide".
...

{
  ...
  "OD": {
    ...
    "biometry": { "al": null, "acd": null, "lt": null, "cct": null, "wtw": null },
...
def _flatten_refraction_ocr(raw: dict) -> dict:
    ...
    def flatten_eye(eye_data):
        ...
        return {
            ...
            'c_ax':  fi(w.get('axis')),
            'al':    fv(b.get('al')),   'acd':   fv(b.get('acd')),
            'lt':    fv(b.get('lt')),   'cct':   fv(b.get('cct')),
            'wtw':   fv(b.get('wtw')),
...
def gemini_parse_ocr_image(image_paths, target='all'):
    ...
    target_instr = {
        ...
        'pentacam':    "ФОКУС: Только PENTACAM/томография. ANT/POST → плюс-нотация (p_ant_c/a, p_post_c/a). TOTAL → минус-нотация (p_tot_c/a, ось слабого меридиана). type='refraction'.\n",

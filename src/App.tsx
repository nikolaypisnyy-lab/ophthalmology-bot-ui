...
function KeyboardHidden({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // Прячем, если высота экрана уменьшилась более чем на 25% (признак клавиатуры)
...
export function App() {
  ...
  useEffect(() => {
    ...
    // Telegram BackButton support (v6.1+)
  ...
  useEffect(() => {
    if (showNewPatientModal) {
      // Logic for when new patient modal is open if needed
    }
  }, [showNewPatientModal]);

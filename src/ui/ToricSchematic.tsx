...
export function ToricSchematic({ eye, incisionAx, toricAx, steepAx, size = 240 }: Props) {
  void useClinicStore(s => s.theme); // ре-рендер при смене темы
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', overflow: 'visible' }}>
      {/* ── LIMBUS ── */}
      <circle cx={cx} cy={cy} r={limR} fill="none" stroke={C.border2} strokeWidth={1.5} />
      ...
      {toricAx != null && (
        <g transform={`translate(${cx}, ${cy}) rotate(${iolRot})`}>
          <circle cx={0} cy={0} r={oR} fill={C.surface3} stroke={C.muted2} strokeWidth={1.5} />
          {[oR - 8, oR - 16].map(x => (
            <React.Fragment key={x}>
              <circle cx={x} cy={0} r={2.5} fill={C.text} />
              <circle cx={-x} cy={0} r={2.5} fill={C.text} />
            </React.Fragment>
          ))}
        </g>
      ...
      {toricAx == null && (
        <>
          <circle cx={cx} cy={cy} r={limR * 0.4} fill={C.surface2} stroke={C.border} strokeWidth={1.5} strokeDasharray="5 4" />
          <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fill={C.muted3} fontSize={16} fontFamily={F.sans}>CALC</text>
        </>
      )}
    </svg>
  );

...
def handle_nuclear_backup(call):
    ...
    bot.send_message(uid, 'ð Запуск полного архивирования системы... Подождите.')
    try:
        import subprocess
        import datetime
        now = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')
        archive_path = f'/root/medeye_full_backup_{now}.tar.gz'
        # Архивируем всё: код, данные, логи и конфиги сервисов
        cmd = f'tar -czf {archive_path} /root/medeye /etc/systemd/system/medeye*'
        subprocess.run(cmd, shell=True, check=True)
        
        file_size = os.path.getsize(archive_path) / (1024 * 1024)
        if file_size > 49:
            bot.send_message(uid, f'⚠️ Файл слишком велик ({file_size:.1f}MB) для отправки через бота. Он сохранен на сервере: {archive_path}')
        else:
            with open(archive_path, 'rb') as f:
                bot.send_document(uid, f, caption=f'ð ПОЛНЫЙ БЭКАП СИСТЕМЫ ({file_size:.1f}MB)\nДата: {now}')
            os.remove(archive_path)
        bot.answer_callback_query(call.id, '✅ Готово')
    except Exception as e:
        bot.send_message(uid, f'❌ Ошибка архивирования: {e}')
...
def start_cmd(message):
    ...
    bot.send_message(uid, f"ð Приветствуем, {u['name']}!\nКлиника: <b>{u['clinic_name']}</b>", 

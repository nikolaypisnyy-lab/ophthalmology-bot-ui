spawn ssh root@92.38.48.231 cat /root/medeye/api/bot_slim_v2.6.py
root@92.38.48.231's password: 
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def main_menu_markup(uid: int):
    ...
    kb.row(types.KeyboardButton("ð Открыть RefMaster", web_app=types.WebAppInfo(url)))
    
    clinics = master_db.get_user_clinics(uid)
    if len(clinics) > 1:
        kb.row("ð¥ Сменить клинику")
    
    if cid:
        u = master_db.get_user_clinic(uid, cid)
        if u and (u["role"] == "admin" or uid in ADMIN_IDS):
            kb.row("ð¥ Управление доступом")
            kb.row("⚙️ Управление клиниками", "ð¦ Бэкап базы")
...
def handle_delete_backup(call):
    ...
    try:
        if os.path.exists(path):
            ...
            bot.edit_message_text(call.message.text + '\n\nð️ <b>Архив удален с сервера.</b>', call.message.chat.id, call.message.message_id)
        else:
...
def handle_nuclear_backup(call):
    uid = call.from_user.id
    if uid not in ADMIN_IDS: return
    bot.send_message(uid, 'ð Запуск полного архивирования... Пожалуйста, подождите.')
    try:
        ...
        text = f'ð <b>ГЕНЕРАЛЬНЫЙ БЭКАП ГОТОВ!</b>\n\nРазмер: {file_size:.1f}MB\n\nСкачать по прямой ссылке:\n{download_url}'
...
def handle_backup_access(call):
    ...
    try:
        ...
        with open(path, 'rb') as f:
            bot.send_document(uid, f, caption='ð¡️ Бэкап прав доступа (JSON)')
...
def admin_secret_menu(message):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("ð ЯДЕРНЫЙ БЭКАП", callback_data="admin_nuclear_backup"),
        types.InlineKeyboardButton("ð¡️ Права доступа", callback_data="admin_backup_access")
    )
    kb.add(types.InlineKeyboardButton("ð¦ Полный бэкап системный", callback_data="admin_full_backup_run"))
    
    bot.send_message(uid, "<b>ð ️ Терминал супер-администратора:</b>", reply_markup=kb)
...
def handle_restore_document(message):
    ...
    if doc.file_name.endswith('.tar.gz') and 'medeye_nuclear' in doc.file_name:
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton('ð¥ ДА, ВОССТАНОВИТЬ ВСЁ', callback_data=f'confirm_restore:{doc.file_id}'))
...
def start_cmd(message):
    ...
    if not u:
        text = (
            "ð Добро пожаловать в <b>RefMaster</b>.\n\n"
        ...
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("ð Запросить доступ", callback_data=f"req_access:{uid}"))
        bot.send_message(uid, text, reply_markup=kb)
        return

        roles = {"admin": "Администратор", "surgeon": "Хирург", "diagnostic": "Диагност"}
    role_label = roles.get(u['role'], u['role'])
    text = f"ð Приветствуем, {u['name']}.
...
def handle_admin_grant(c):
    ...
    bot.send_message(target_uid, f"ð Вам выдан доступ в клинику <b>{cid}</b>!\nРоль: <b>{role}</b>\n\nТеперь вы можете открыть WebApp.", 
...
def admin_clinics(message):
    ...
    for c in clinics:
        ...
        kb.row(
            types.InlineKeyboardButton(f"ð Имя", callback_data=f"adm_ren_init:{c['clinic_id']}"),
...
def admin_delete_init(c):
    ...
    kb.row(
        ...
        types.InlineKeyboardButton("ð Отмена", callback_data="admin_clinics_refresh")
    )
...
@bot.message_handler(func=lambda m: m.text == "ð¥ Управление доступом")
...
def handle_revoke_link(message):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("ð ОТЗВАТЬ ДОСТУП", callback_data=f"adm_revoke_cfm:{uid}:{cid}"),
        types.InlineKeyboardButton("ð Отмена", callback_data="admin_users_refresh")
    )
...
def admin_revoke_confirm(c):
    ...
    try: bot.send_message(uid, f"ð Ваш доступ в клинику <b>{cid}</b> был отозван администратором.")
    except: pass
...
def admin_users_refresh(c):
    ...
    class FakeMsg:
        def __init__(self, c):
            self.from_user = c.from_user
            self.chat = c.message.chat
            self.text = "ð¥ Управление доступом"
    admin_users(FakeMsg(c))

@bot.message_handler(func=lambda m: m.text == "ð¥ Сменить клинику" or m.text == "/clinics")
...
@bot.message_handler(func=lambda m: m.text == "ð¦ Бэкап базы")
def admin_backup(message):
    ...
    for p in paths:
        if os.path.exists(p):
            with open(p, "rb") as f:
                bot.send_document(message.chat.id, f, caption=f"ð Бэкап {cl['name']}")
                found = True
                break
    if not found:
